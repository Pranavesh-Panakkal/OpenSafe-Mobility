...
from .exceptions import (
    ...
    FileNotFound,
    CurrentPlanNotRun,
    CrossSectionNotFound,
    CulvertNotFound,
    BridgeNotFound,
    MultipleOpeningNotFound,
    InlineStructureNotFound,
    LateralStructureNotFound,
    RCException,
    NoProject,
    RASOpen,
)
...
class River(object):
    ...
    def _update_reach_codes(self):
        ...
        if self.rc.output_getreaches(self.code) == self.rc.geometry_getreaches(
            self.code
        ):
            pass
        else:
...
class Node(object):
    ...
    def value(self, profile, value_type):
        ...
        return self.rc.output_nodeoutput(
            self.river.code, self.reach.code, self.code, profile.code, value_type
        )
    ...
    def __repr__(self):
        ...
        return 'Node name = "{}"/Node type = "{}"/Node code = "{}"'.format(
            self.node_id, node_type, self.code
        )
...
class RasController(object):
    ...
    def __init__(self, version="506"):
        ...
        self.project_is_open = (
            False  # has a project been opened? set by self.open_project()
        )
        ...
        for p in psutil.process_iter():
            try:
                if p.name().lower() == "ras.exe":
                    raise RASOpen(
                        "HEC-RAS appears to be open. Please close HEC-RAS. Exiting."
                    )
            except psutil.Error:
                # TODO: This should be handled better.
                pass
    ...
    def _simple_node_list(self, node_type):
        ...
        if not self.project_is_open:
            raise NoProject(
                "Project must be opened before calling RasController._simple_node_list()"
            )
        ...
        if node_type == "xs":
            ...
            for node in self.xs_list:
                temp = SimpleXS(
                    xs_id=node.node_id.strip(),
                    river=node.river.name.strip(),
                    reach=node.reach.name.strip(),
                )
    ...
    def _get_node(self, node_type, node_id, river=None, reach=None):
        ...
        for node in node_list:
            if river is None and reach is None:
                ...
                if (
                    node.node_id.strip() == node_id
                    and node.river.name.strip() == river
                    and node.reach.name.strip() == reach
                ):
                    return node
        ...
        if node_type == "xs":
            ...
            raise MultipleOpeningNotFound(
                "Multiple opening " + str(node_id) + " not found"
            )
        elif node_type == "inline_struct":
            raise InlineStructureNotFound(
                "Inline structure " + str(node_id) + " not found"
            )
        elif node_type == "lateral_struct":
            raise LateralStructureNotFound(
                "Lateral structure " + str(node_id) + " not found"
            )
    ...
    def set_plan(self, plan):
        ...
        if self._plan_lock:
            raise LockedPlan(
                "The plan can not be changed after running get_profiles()."
            )
    ...
    def read_compute_msg(self, plan):
        ...
        if self._model_ran is False:
            raise CurrentPlanNotRun(
                "Run the current plan before reading the compute message."
            )
        if not os.path.isfile(compute_msg_file):
            raise FileNotFound(
                "{} does not exist. The model needs to run for this file to be created".format(
                    compute_msg_file
                )
            )
    ...
    def get_rivers(self):
        ...
        if river_names is None:
            raise NoOutputFile(
                "Output file does not appear to exist. Model may not have run successfully."
            )
    ...
    def is_output_current(self, plan, show=False):
        ...
        """
        result, plan_name, unknown_bool, message = self.com_rc.PlanOutput_IsCurrent(
            plan.name, show, None
        )
        if DEBUG:
    ...
    def geometry_getnodes(self, river_id, reach_id):
        ...
        """
        _, _, _, node_ids, node_types = self.com_rc.Geometry_GetNodes(
            river_id, reach_id, None, None, None
        )
    ...
    def output_nodeoutput(self, river_id, reach_id, node_id, profile, value_type):
        ...
        value = self.com_rc.Output_NodeOutput(
            river_id, reach_id, node_id, 0, profile, value_type
        )[0]
        return value
